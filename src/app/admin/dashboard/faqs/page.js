...
export default function FaqManagementPage() {
  ...
  async function updatePublishState(nextPublished) {
    try {
      const data = await apiRequest(`/api/admin/faqs/${selectedPage}/publish`, {
        method: "PATCH",
        body: JSON.stringify({ isPublished: nextPublished }),
      });
      if (!data?.success) throw new Error("Unable to update FAQ publishing.");
      setIsPublished(data.isPublished);
      setNotice({ type: "success", text: nextPublished ? "All five FAQs are now published." : "FAQs are now saved as a draft." });
    } catch (error) {
      setNotice({ type: "error", text: error.message || "Unable to update FAQ publishing." });
    }
  }
  ...
  async function saveFaq(event) {
    ...
    try {
      ...
      const data = await apiRequest(
        editingId ? `/api/admin/faqs/${selectedPage}/${editingId}` : `/api/admin/faqs/${selectedPage}`,
        { method: editingId ? "PUT" : "POST", body: JSON.stringify({ question: form.question.trim(), answer: form.answer.trim() }) },
      );
  ...
  async function deleteFaq(faq) {
    ...
    try {
      ...
      setNotice({ type: "error", text: error.message || "Unable to delete FAQ." });
    }
  }
  ...
  async function moveFaq(index, direction) {
    ...
    try {
      ...
      await loadFaqs();
      setNotice({ type: "error", text: error.message || "Unable to reorder FAQs." });
    }
  }

  return (
    <div className="max-w-5xl">
      <div className="mb-8"><h1 className="text-3xl font-semibold text-gray-900">FAQ Management</h1><p className="mt-2 text-gray-500">Manage up to five FAQs for each service page.</p></div>
      <section className="rounded-xl border border-gray-200 bg-white p-6">
        ...
        <label className="block max-w-xl text-sm font-medium text-gray-700">Service page
          <select value={selectedPage} onChange={(event) => { setSelectedPage(event.target.value); resetForm(); setNotice(null); }} className="mt-2 w-full rounded-lg border border-gray-300 bg-white px-4 py-3 text-gray-900 outline-none focus:border-[#0A4E08]">
            <option value="home">Home Page</option>
            {serviceSections.map((section) => <optgroup key={section.value} label={section.label}>{section.categories.map((category) => category.links.map((link) => <option key={link.slug} value={link.slug}>{category.title} — {link.label}</option>))}</optgroup>)}
          </select>
        </label>
        ...
        <form onSubmit={saveFaq} className="mt-7 rounded-xl border border-gray-200 bg-gray-50 p-5">
          ...
          <label className="mt-4 block text-sm font-medium text-gray-700">Answer
            <textarea value={form.answer} maxLength={5000} rows={5} onChange={(event) => setForm({ ...form, answer: event.target.value })} className="mt-2 w-full rounded-lg border border-gray-300 bg-white px-4 py-3 text-gray-900 outline-none focus:border-[#0A4E08]" required />
          </label>
